...
class MainActivity : AppCompatActivity(), QuestionFragment.OnQuestionFragmentListener{
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            showFragment(QuestionFragment(), true)
        }
    }
    ...
    override fun onBackPressed() {
        if (currentPage == 1) {
            ...
            when (fragment) {
                is QuestionFragment -> showFragment(fragment, false)
                else -> finish()
            }
        }
    }
    ...
    override fun setFragmentTheme() {
        val statusBarColor = when(currentPage){
            ...
            3 -> R.color.light_green_100_dark
            4 -> R.color.deep_purple_100_dark
            else -> R.color.cyan_100_dark
        }
    ...
    override fun nextPage() {
        if (currentPage == ResultFragment.MAX_QUESTIONS) {
            ...
            for (entry in 1..supportFragmentManager.backStackEntryCount) {
                val fragment = supportFragmentManager.findFragmentByTag("f$entry") as QuestionFragment
                list.add(fragment.userChoice)
            }
